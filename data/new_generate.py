...
for i in range(0,numOrders-2,3):
    ...
    total_price = sum([menu[x] for x in myitems])
    ...
    for x in myitems:
        order_item.write(f"{i},{x},{1},{total_price}\n")
    ...
    for x in myitems:
        order_item.write(f"{i+1},{x},{1},{total_price}\n")
        if random.randint(0,1)==1:
    ...
    for x in myitems:
        order_item.write(f"{i+2},{x},{1},{total_price}\n")
        if random.randint(0,1)==1:
