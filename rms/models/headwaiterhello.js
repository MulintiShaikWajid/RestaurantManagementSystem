...
module.exports = class Headwaiterhello{
    ...
    static get_table_requests(){
    	return pool.query("select username, name, table_id, requested_time, start_time, end_time, request_id from table_request B, person \
        where customer_id=id and status='request-placed' and not exists(select A.request_id from table_request A \
        where A.request_id != B.request_id and A.start_time = B.start_time and A.end_time = B.end_time and \
